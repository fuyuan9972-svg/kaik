...
fn sample_calibrated_17_v2_prompt(
    ...
    format!(
        "{base}\n\n17% 2.0 细调：在不改变上述规则的前提下，比成功链路基线稍微再生涩一点。少用过顺的表达，例如“对于……也有一些作用”“更为紧密一些”“产生作用”；可改成更普通、更笨一点的说法，例如“对……有一点帮助”“结合得更充分一些”“能起到一些帮助”。如果段落属于人工智能、证券金融、智能投顾、智能风控、监管治理、案例对比这类题材，且写成“概念定义+机制说明+例子+价值总结”或“第一第二第三第四”的完整解释包，要优先拆掉闭环：删弱总括句，保留事实和数据，把作用、风险、对策分成不完全对称的短说明，避免每句都写成标准教材式定义。不要大幅扩写，不要把每段写成解释性长段，不要堆叠“比较、一些、会、情况、过程中”等词。单次长度仍按基线控制，不能为了降低 AIGC 增加新事实、例子、数据或结论。只输出最终段落纯文本。"
    )
}
